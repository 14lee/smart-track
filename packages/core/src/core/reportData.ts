...
import { SDK_VERSION, EVENT_TYPES } from '@smart-track/common';
import { ReportData, InitOptions } from '@smart-track/types';
...
export class TransportData {
  ...
  getTransportData(data: any): ReportData {
    ...
    const excludePerformanceRecordWhiteScreen = [
      EVENT_TYPES.PERFORMANCE,
      EVENT_TYPES.RECORD_SCREEN,
      EVENT_TYPES.WHITE_SCREEN,
    ];
    if (!excludePerformanceRecordWhiteScreen.includes(data.type)) {
  ...
  async send(data: ReportData) {
    ...
    // 开启录屏，由@smart-track/rack/recordScreen 插件控制
